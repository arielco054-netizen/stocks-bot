...
TICKERS = {
    ...
    "PRK": ("פרוק", "ProK"),  # תיקון הסימול לפרוק
...
def fetch_ticker_data(ticker, hebrew_name, is_saturday):
    try:
        ...
        if len(history) < 2:
            print(f"⚠️ אזהרה: לא נמצאו מספיק נתונים עבור הסימול {ticker}")
            return None
...
def main():
    ...
    def format_block(items, start_index=1):
        ...
        for i, item in enumerate(items, start=start_index):
            ...
            if "BTC" in item['ticker']:
                price_suffix = " USD"
            elif "TA125" in item['ticker']:
                price_suffix = " נקודות"
            else:
                price_suffix = "$"
            
            line = (
                f"<b>{i}.</b> 📊 {emoji} {item['hebrew_name']} | {TICKERS[item['ticker']][1]}\n"
